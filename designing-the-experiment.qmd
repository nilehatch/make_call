...
```{r}
#| label: fig-generate-ladder
#| fig-cap: "You are here, at the top of the evidence ladder. The lower rungs gathered what already existed; now you generate the strongest evidence there is, what your customer does in a test you design."
#| echo: false
#| fig-width: 8
#| fig-height: 3.5
#| out-width: "760px"
...
rungs <- data.frame(
  ...
  fill    = c("#F3F4F6", "#F3F4F6", "#C4DEEB", "#A9D0E0"),
  namecol = c("#AEB3BA", "#AEB3BA", "#0B3C52", "#0B3C52"),
  cellcol = c("#AEB3BA", "#AEB3BA", "#1F2937", "#1F2937")
)
ggplot(rungs) +
  geom_rect(aes(xmin = 0, xmax = 12.9, ymin = y - 0.42, ymax = y + 0.42, fill = fill),
            color = "white", linewidth = 1) +
  scale_fill_identity() +
  geom_text(aes(x = 0.3, y = y, label = name), hjust = 0, fontface = 2,
            size = 4.3, color = rungs$namecol) +
  geom_text(aes(x = 3.6, y = y, label = ex), hjust = 0, size = 4.0, color = rungs$cellcol) +
  geom_text(aes(x = 6.9, y = y, label = cost), hjust = 0, size = 4.0, color = rungs$cellcol) +
  geom_text(aes(x = 9.9, y = y, label = use), hjust = 0, size = 4.0, color = rungs$cellcol) +
...
The mistake is to run a confirmatory test on an unexplored question. You will get a crisp, confident answer to the wrong question: efficient failure. If you cannot yet write a grounded hypothesis (you saw how in the chapter on [writing a proper hypothesis](hypothesizing.qmd), and how to build the grounding through [exploration](exploring.qmd)), your next experiment is an exploration, not a test. Confirm only what exploration has earned.
...
You don't clear the fog by walking into it until you hit something. You clear it one honest question at a time. Next, we turn from running the test to [reading it](weighing-the-evidence.qmd): how to weigh whether the evidence is credible, and what it should do to what you believe.
