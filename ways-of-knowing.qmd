...
The [last chapter](uncertainty.qmd) said the valuable peaks are out in the fog, waiting for someone to find them. This one asks the harder question that follows: once you are out there, past the mapped ground, how do you *know* anything at all? When certainty is gone, what separates knowing from guessing, and how do you tell them apart in the moment, with money on the line?
...
Knowing is not a switch but a dial. A **thin** knowing is a hunch or a gut pull. A **thicker** knowing has been checked against evidence you actually gathered. And turned all the way up is **conviction**, a belief you hold past what the evidence can yet prove. Each has its place. The danger is confusing them — acting on a hunch as though it were tested, or throwing out a conviction because you cannot put a number on it. Much of deciding well under uncertainty is just being honest about which grade of knowing you really have.

## The edge of the map {#sec-edge}

Here is the same terrain — the known ground and the fog beyond it — drawn a different way. Picture everything that could possibly be known as a vast circle. Human knowledge, everything our whole species has ever worked out, is a small speck inside it. Your own knowledge is a speck within that speck. Most of what there is to know, no one knows yet.[^might]
...
library(ggplot2)
library(ggforce)
light    <- "#E9F3F6"   # could-be-known (pale)
mid      <- "#6FB1CE"   # transition: could-be becoming known
teal     <- "#007BA7"   # is-known
darkteal <- "#005E7F"   # borders
grey <- "#6B7280"; ink <- "#1F2937"; fog <- "#FBFDFE"

# panel 1: the whole circle
big   <- data.frame(x0 = 2.5, y0 = 2.7, r = 2.1)
speck <- data.frame(x0 = 3.45, y0 = 3.65, r = 0.30)

# panel 2: zoom on the speck's edge (centered on the frame so margins are even)
frame <- data.frame(xmin = 5.7, xmax = 9.9, ymin = 0.8, ymax = 4.6)
cx <- 7.8; cy <- -3.9; cr <- 7.0
xs <- seq(frame$xmin, frame$xmax, length.out = 120)
ys <- cy + sqrt(pmax(0, cr^2 - (xs - cx)^2))
arc <- data.frame(x = xs, y = ys)
known_poly <- rbind(data.frame(x = frame$xmin, y = frame$ymin), arc, data.frame(x = frame$xmax, y = frame$ymin))
topy <- cy + cr; br <- 0.46
bx <- seq(-br, br, length.out = 48); by <- sqrt(pmax(0, br^2 - bx^2))
bump      <- data.frame(x = cx + bx, y = topy + by)
bump_fill <- rbind(data.frame(x = cx - br, y = topy), data.frame(x = cx + bx, y = topy + by), data.frame(x = cx + br, y = topy))

ggplot() +
  annotate("segment", x = 3.66, y = 3.90, xend = 5.7, yend = 4.6, linetype = "dashed", color = grey, linewidth = 0.4) +
  annotate("segment", x = 3.66, y = 3.40, xend = 5.7, yend = 0.8, linetype = "dashed", color = grey, linewidth = 0.4) +
  geom_circle(data = big, aes(x0 = x0, y0 = y0, r = r), fill = light, color = teal, linewidth = 1.0) +
  geom_circle(data = speck, aes(x0 = x0, y0 = y0, r = r), fill = teal, color = darkteal, linewidth = 0.6) +
  ...
  annotate("segment", x = 2.62, y = 4.12, xend = 3.25, yend = 3.75, color = darkteal, linewidth = 0.4) +
  geom_rect(data = frame, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), fill = fog, color = NA) +
  geom_polygon(data = known_poly, aes(x, y), fill = teal) +
  geom_polygon(data = bump_fill, aes(x, y), fill = mid) +
  geom_path(data = arc, aes(x, y), color = darkteal, linewidth = 1.1) +
  ...
  annotate("text", x = cx, y = 1.7, label = "the known", color = "white", size = 3.8, fontface = 2) +
...
::: {.column-margin .def-margin}
**Premise** — a starting assumption a piece of reasoning takes for granted. If the premise is wrong, everything built on it is wrong too, however clean the logic.
:::
...
A founder imagines a booming market for smart toasters: busy parents wanting fresh bread timed to their mornings. The story drives the pitch and the prototype. Then they run real surveys and find almost no one wants to upgrade a toaster. The vision was vivid. Reality wasn't. *Imagination is the spark; it only matters if you test the fire it lights.*
:::
...
You now know what knowing is, and how to do it honestly past the edge of the map. It comes down to one move: hold your imperfect lenses up together and read them against each other. That is the method's whole epistemic engine. But knowing *how* to do a thing is not the same as being able to make yourself do it, and staying in the fog long enough to cross-check, instead of grabbing the first lens that soothes you, turns out to be one of the hardest things a founder ever does. The last thing to understand before we walk the method itself is [why that is so hard](why-the-fog-hurts.qmd), and what it costs when you fail.

[^might]: This picture is adapted from Matt Might's "Illustrated Guide to a PhD" [@mightIllustratedGuidePhD2010], where all human knowledge is a circle and a doctorate is the tiny bump you push outward at its edge. The version here is our own reworking of his idea, not his drawing.
