...
A hypothesis cannot be plucked from thin air. To be worth testing, it has to rest on some real understanding of the thing it is about — call that **groundedness**, the amount of genuine contact with reality behind a belief. A belief with high groundedness is built from things you have actually seen or lived through; one with low groundedness is built from what you assume, hope, or half-remember.

::: {.column-margin .def-margin}
**Groundedness** — how much real contact with the world a belief rests on. Low groundedness is a hunch; high groundedness is a claim shaped by what you have actually seen, heard, or lived.
:::
...
#| fig-cap: "The groundedness threshold. When uncertainty runs ahead of what you have actually observed, you sit in the *explore* region and any hypothesis is a guess. Exploration builds groundedness and carries you across the line, into territory where a claim is finally worth testing."
#| echo: false
#| warning: false
#| message: false
#| fig-width: 6
...
#| out-width: "540px"
library(ggplot2)
explore <- data.frame(x = c(0, 0, 1), y = c(0, 1, 1))
hypo    <- data.frame(x = c(0, 1, 1), y = c(0, 0, 1))
lab     <- data.frame(x = 0.50, y = 0.485, t = "exploration builds groundedness")
ggplot() +
  geom_polygon(data = explore, aes(x, y), fill = "#FBF0D9") +
  geom_polygon(data = hypo,    aes(x, y), fill = "#E6F2F7") +
  geom_segment(aes(x = 0, y = 0, xend = 1, yend = 1), color = "#007BA7", linewidth = 1.6) +
  annotate("text", x = 0.30, y = 0.74, label = "Explore", color = "#B8860B", fontface = 2, size = 6.5) +
  annotate("text", x = 0.30, y = 0.63, label = "grounding too thin\nfor the uncertainty", color = "#8A6D1F", size = 3.3, lineheight = 0.95) +
  annotate("text", x = 0.70, y = 0.26, label = "Hypothesize & test", color = "#006B90", fontface = 2, size = 6.5) +
  annotate("text", x = 0.70, y = 0.15, label = "grounded enough\nto risk a claim", color = "#2B6A86", size = 3.3, lineheight = 0.95) +
  annotate("segment", x = 0.28, y = 0.42, xend = 0.72, yend = 0.42, color = "#1F2937", linewidth = 0.7, arrow = arrow(length = unit(0.18, "cm"), type = "closed")) +
  geom_label(data = lab, aes(x, y, label = t), fill = "white", color = "#1F2937", size = 3, fontface = 3,
             label.size = 0, label.r = unit(0.5, "lines"), label.padding = unit(0.45, "lines")) +
  scale_x_continuous("Groundedness  (low → high)", limits = c(0, 1), expand = c(0, 0)) +
  scale_y_continuous("Uncertainty  (low → high)", limits = c(0, 1), expand = c(0, 0)) +
  coord_fixed() +
  theme_minimal(base_size = 13) +
  theme(axis.text = element_blank(), axis.ticks = element_blank(),
        panel.grid = element_blank(),
        axis.title = element_text(face = 2, color = "#374151"),
        plot.margin = margin(6, 10, 6, 6))
```

## What exploring looks like {#sec-forms}

Exploration is structured wandering. The philosopher Ian Hacking called this kind of inquiry *adventure* [-@hackingEmergenceProbabilityPhilosophical2006]: deliberately stepping into the world to experience it and let it reshape your beliefs, rather than to confirm the ones you walked in with. You go out without a hypothesis on purpose, because a hypothesis this early would only tell you what to ignore.

Exploration takes a few familiar forms, and most mix them.
...
Exploration ends with something in your hand: a surprise, a pattern, a signal that the frame you started with was not quite right. What it does not yet give you is a claim. Turning a signal into a sharp, testable hypothesis, making the leap from *I noticed this* to *I bet that*, is [a move with a name and a logic of its own](hypothesizing.qmd), and it is the last step of framing before the world gets to answer back. That is where we turn next.
