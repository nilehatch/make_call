...
```{r}
#| label: fig-evidence-gradient
#| fig-cap: "The evidence gradient. Already-existing sources climb from what people *said* (cheap, weak) toward what they *did* (costlier, stronger). The strongest evidence of all — what your customer does in a test you design — sits past the edge of what is already known. You have to generate it."
#| echo: false
#| fig-width: 9
#| fig-height: 3
#| out-width: "720px"
library(ggplot2)
band <- data.frame(x = seq(0.3, 8.4, length.out = 300))
pts <- data.frame(
  x = c(1.3, 3.4, 5.5, 7.6),
  lab = c("Published\ndata", "Human\nexperts", "Market &\ncompetitor\nsignals", "Your own\nnumbers")
)
gen <- data.frame(x = 9.7, lab = "Designed\nexperiments")
ggplot() +
  geom_tile(data = band, aes(x = x, y = 1, fill = x), width = 0.04, height = 0.5) +
  scale_fill_gradient(low = "#EAF4F7", high = "#007BA7", guide = "none") +
  annotate("segment", x = 8.4, xend = 8.4, y = 0.55, yend = 1.5,
           linetype = "dashed", color = "#9CA3AF") +
  geom_point(data = pts, aes(x, 1), size = 5, color = "white") +
  geom_point(data = pts, aes(x, 1), size = 3.3, color = "#007BA7") +
  geom_text(data = pts, aes(x, 1.36, label = lab), size = 3,
            lineheight = 0.9, fontface = 2, color = "#1F2937") +
  geom_point(data = gen, aes(x, 1), size = 5, color = "white") +
  geom_point(data = gen, aes(x, 1), size = 3.3, color = "#E3A008") +
  geom_text(data = gen, aes(x, 1.36, label = lab), size = 3,
            lineheight = 0.9, fontface = 2, color = "#8A6D1A") +
  annotate("text", x = 0.3, y = 0.32, hjust = 0,
           label = "what people SAID  ·  cheap  ·  weak", size = 2.9,
           fontface = 3, color = "#6B7280") +
  annotate("text", x = 8.4, y = 0.32, hjust = 1,
           label = "what people DID  ·  costly  ·  strong", size = 2.9,
           fontface = 3, color = "#374151") +
  annotate("text", x = 4.35, y = 1.74, label = "GATHER WHAT'S ALREADY KNOWN",
           size = 3, fontface = 2, color = "#007BA7") +
  annotate("text", x = 9.7, y = 1.74, label = "GENERATE",
           size = 3, fontface = 2, color = "#8A6D1A") +
  coord_cartesian(xlim = c(0, 11), ylim = c(0.2, 1.9), expand = FALSE) +
  theme_void()
```
