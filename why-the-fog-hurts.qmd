...
That is the trade at the center of working in uncertainty: *I do not know where I am headed, but I know what to do next, and I trust that doing the next right thing will take me somewhere worth being.* It is a strange way to live, and it does not come naturally to everyone; the rule-driven, destination-first mind finds it genuinely hard, and squirms. But it is the entrepreneur's native stance, and it can be learned. It does not erase the pain. It takes the edge off — and there is a quiet reward waiting past it. When you stop straining to see the destination and pour yourself instead into the work of the next step, the discomfort often disappears altogether, dissolved into the plain absorption of doing — the *flow* a well-matched task can bring.[^flow] You will not always feel it lift. But over a career of steps taken in the fog, you also slowly get used to the fog itself, which is a change we will come back to at the very end of the book.
...
- **Have it check the borrowed map.** Describe the model you are leaning on and where it came from. Then ask what about your actual situation might not match. It is good at spotting a theory laid over the wrong world.
...
[^psych]: The three run together in the research literature: an *intolerance of uncertainty* [@freestonWhyPeopleWorry1994]; an aversion to *ambiguity*, our preference for a known risk over an unknown one, named for Daniel Ellsberg's paradox [@ellsbergRiskAmbiguitySavage1961]; and a *need for cognitive closure* [@kruglanskiMotivatedClosingMind1996].
...
[^flow]: The absorbing, self-forgetting focus a well-matched task can produce is what @csikszentmihalyiFlowPsychologyHappiness2013 named *flow*.
